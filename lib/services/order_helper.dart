...
// Add to cart HELPER
  ...
  static Future<bool> addToCart(AddToCart model) async {
    ...
    Map<String, String> requestHeaders = {
      ...
      'Content-Length': modeldata.length.toString(), // Add the Content-Length header
      "Accept": "application/json",
      "Host": "'exceedingproperties-production.up.railway.app'",
    };

    var url = Uri.https('exceedingproperties-production.up.railway.app', Config.addCartUrl);
  ...
  static Future<AddCartResponse> newAddtoCart(AddToCart model) async {
    ...
    var url = Uri.parse('https://exceedingproperties-production.up.railway.app/cart');
    var headers = {
      'Content-Type': 'application/json',
      'Authorization': 'Bearer $token'
    };
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        AddCartResponse addresponse = AddCartResponse.fromJson(jsonDecode(response.body));
  ...
  static Future<bool> addingToDart(AddToCart model) async {
    ...
    var headers = {
      "Accept": "application/json",
      'Content-Type': 'application/json',
      'Content-Length': jsonData.length.toString(), // Add the Content-Length header
      'Authorization': 'Bearer $token',
    };
  ...
  void makePostRequest() async {
    ...
    String url = "https://${Config.apiUrl}${Config.addCartUrl}"; // Replace with your API endpoint
  ...
  static Future<List<MProducts>?> getCartProduct() async {
    ...
    if (response.statusCode == 200) {
      ...
      GetCartResponse cartResponse = GetCartResponse.fromJson(jsonDecode(response.body));
      List<MProducts>? cart = [];
